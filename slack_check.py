...
def schedule_tasks():
    ...
    # 오전 10시, 11시 체크
    scheduler.add_job(morning_check, 'cron', hour=10, minute=0)
    scheduler.add_job(morning_check, 'cron', hour=11, minute=0)

    # 오후 10시, 11시 체크
    scheduler.add_job(afternoon_check, 'cron', hour=22, minute=0)
    scheduler.add_job(afternoon_check, 'cron', hour=23, minute=0)
