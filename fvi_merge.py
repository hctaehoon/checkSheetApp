...
def update_fvi_excel(date, team, worker, manager, items):
    # 날짜에서 년과 월을 추출
    ...
    # 팀별 폴더 설정
    target_directory = f"./excel/fvi/{team}"
    os.makedirs(target_directory, exist_ok=True)
    
    # 파일 이름 생성 및 경로 설정
    filename = f"FVI_{int(month)}월.xlsx"
    ...
    if os.path.exists(filepath):
        ...
        # 파일이 없으면 기본 템플릿 파일(FVI.xlsx)을 복사하여 새로 생성
        shutil.copyfile("./checksheet/FVI.xlsx", filepath)
        workbook = load_workbook(filename=filepath)

    # 첫 번째 시트 업데이트
    sheet1 = workbook["DMTI-FVI-01-07 (FVI 설비 및 청소 점검)"]
    sheet1["AB3"] = f"{year}년 {int(month)}월"
    sheet1["F2"] = f"FVI ( {team}조)"
    column_index = 7 + (day - 1)
    rows_to_update_sheet1 = list(range(7, 32, 2))  # 7, 9, 11, ..., 31
    for row in rows_to_update_sheet1:
        if is_merged_cell(sheet1, row, column_index):
            sheet1.cell(row=row, column=column_index).value = 'O'

    # 두 번째 시트 업데이트
    sheet2 = workbook["DMTI-FVI-01-07 (FVI PM 검증)"]
    sheet2["AB3"] = f"{year}년 {int(month)}월"
    sheet2["F2"] = f"FVI ( {team}조)"
    rows_to_update_sheet2 = list(range(7, 46, 2))  # 7, 9, 11, ..., 45
    ...
    if os.path.exists(replace_log_filepath):
        with open(replace_log_filepath, 'r') as replace_log_file:
            ...
            for line in replace_logs:
                if "G33" in line:
                    sheet1["G33"] = line.strip().replace("G33", "")
                if "G34" in line:
                    sheet1["G34"] = line.strip().replace("G34", "")
                if "G35" in line:
                    sheet1["G35"] = line.strip().replace("G35", "")

    if items.get(11) == 1:  # item_id 11이 1이면 G33 교체일 갱신
        ...
        sheet1["G33"] = f"{recent_date}   {next_date_str}"
        with open(replace_log_filepath, 'a') as replace_log_file: 
            replace_log_file.write(f"G33 {recent_date}   {next_date_str}\n")

    if items.get(12) == 1:  # item_id 12가 1이면 G34 교체일 갱신
        recent_date = f"최근 교체일: {date}"
        next_date = (datetime.strptime(date, "%y%m%d") + timedelta(days=90)).strftime("%y%m%d")
        next_date_str = f"다음 교체일: {next_date}"
        sheet1["G34"] = f"{recent_date}   {next_date_str}"
        with open(replace_log_filepath, 'a') as replace_log_file:
            replace_log_file.write(f"G34 {recent_date}   {next_date_str}\n")

    if items.get(13) == 1:  # item_id 13이 1이면 G35 교체일 갱신
        recent_date = f"최근 교체일: {date}"
        next_date = (datetime.strptime(date, "%y%m%d") + timedelta(days=365)).strftime("%y%m%d")
        next_date_str = f"다음 교체일: {next_date}"
        sheet1["G35"] = f"{recent_date}   {next_date_str}"
        with open(replace_log_filepath, 'a') as replace_log_file:
            replace_log_file.write(f"G35 {recent_date}   {next_date_str}\n")

    # worker와 manager를 첫 번째 시트의 G36, G37에 입력
    sheet1.cell(row=36, column=column_index).value = worker
    sheet1.cell(row=37, column=column_index).value = manager
...
def fill_missing_dates(data, team, worker, manager):
    ...
    for missing_date in missing_dates:
        ...
        update_fvi_excel(missing_date, team, worker, manager, {last_valid_record['item_id']: last_valid_record['checked']})

# DB 데이터 가져와서 엑셀 업데이트
async def process_fvi_data():
    await database.connect()
    query = "SELECT * FROM fvi_sheet1"
    ...
    for record in data:
        ...
        update_fvi_excel(record['date'], record['team'], record['worker'], record['manager'], items)
